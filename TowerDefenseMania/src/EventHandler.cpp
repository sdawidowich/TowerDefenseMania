...
void EventHandler::check_menu_button_events(sf::RenderWindow& window, sf::Event& event, Menu* menu) {
...
void EventHandler::check_events(sf::RenderWindow& window, sf::Event& event, Game_State& state, Menu* start_menu, Menu* stat_menu, GUI* gui, Environment* environment, std::vector<Tower*>& towers, int& gold) {
	if (state == Game_State::START) {
		this->check_menu_button_events(window, event, start_menu);
	}
	else if (state == Game_State::STATS) {
		this->check_menu_button_events(window, event, stat_menu);
	}
